...
Bnd/Bndtools OSGi workspace (Gradle, Java 21). Milestones 0–2 of `docs/roadmap.md` are implemented: the reworked configuration model (root `DataAtlasConfiguration` with containment registries; a `DataSet` can embed a canonical query of the upstream `org.eclipse.fennec.query` model), and both input slices on the **upstream repository contract** — every `DataInput` materializes as a read-only `ReadRepository` service (`persistence.repository.id` = input id; there is no Data-Atlas-private source SPI). `bootstrap` loads the configuration XMI and registers config objects as OSGi services; `input.file` registers a file-backed `ReadRepository` per `FileDataInput`; `input.jpa` translates a `JPADataInput` into the fennec persistence factory configurations (`fennec.jpa.EORMMappingService` → `fennec.jpa.EMPersistenceUnit` → `fennec.repository.jpa`, see its readme for the mapping table and the mandatory key prefixes); `rest` serves through per-request repository leases with query prepare-gating, `skip`/`top` push-down and HTTP-bound query parameters (fennec codec serialization). Milestone 4 (configuration lifecycle) is implemented: both bootstraps apply configuration changes as an id-keyed diff (`ConfigurationRegistrar`) — file mode reacts to file changes via a Daanse `io.fs.watcher` listener (the pattern matches the FULL path string) and Config-Admin `@Modified`, atlas mode polls via `refresh.interval.ms` (the client cache needs `cache.ttl.ms` coupled to it, or the poll sees the same cached instance forever); staged updates go draft -> transition -> release and need the schemas in BOTH stages; a broken new version fails hard (teardown + loud log), a corrected one recovers. Milestone 3 (Model Atlas config mode) is implemented on the Data Atlas side: `bootstrap` has a second component (`DataAtlasModelAtlasBootstrap`) that retrieves the configuration from a Model Atlas via the model.atlas client's per-scope `ReadableScopeService` and runs the same `ConfigurationRegistrar` pipeline; verified end-to-end, and the atlas mode is covered by docker-gated OSGi ITs against the real `model.atlas:file-snapshot` image (`ModelAtlasModeIntegrationTest`, `TransformationAtlasModeIntegrationTest`, `DcatPublicationAtlasModeIntegrationTest`, `DataAtlasHistoryAtlasModeIntegrationTest`) which run in CI (ubuntu runners have docker; the tests skip where it is missing). Milestone 5 (GeoJSON) is implemented: `GeoJsonDataService`/`GeoJsonDataServiceConfiguration` in the model (geometry source = lon/lat attribute pair OR a passthrough `org.geojson.model` geometry feature, XOR-validated at registration; remaining attributes → properties, EMF id → Feature id), `GeoJsonEndpointConfigurator`+`GeoJsonServiceResource` as REST siblings in the `rest` bundle, serialization through `org.eclipse.fennec.codec.geojson` (`application/geo+json`, content types since emf.codec#168; `@RequireCodecGeoJson` on the configurator since emf.codec#201 — the codec is a bundle-level resolve requirement, no bnd.identity needed). Milestone 6 (data transformations) is implemented: a `DataTransformation` references the QVT-O AST inside a **CompiledUnit document** (`#//@unit`; a bare parsed AST is not XMI-storable — satellites), authored from `.qvto` via `example/trafo/AstGen.java`; the `transformation` bundle binds the DS `QvtoEngine` (since emf.m2x#245 every engine component uses the shared emf.osgi ResourceSet's package registry — type identity with registrar-registered EPackages holds) and copies the WHOLE document at registration; `input.bridge` serves 1:1-transformed objects with skip/top push-down; both bootstraps resolve referenced transformation documents fully (file mode: whole ResourceSet BEFORE the registrar renames package resources — otherwise a second package instance splits type identity); atlas mode names the unit document by an absolute local URI OR by `eobject-registry://<registry>/<key>#<fragment>` — the document lives in a Model Atlas registry (m2x metamodels seeded as schemas; nsURI-based cross-references since emf.m2x#246), the model.atlas `AtlasEObjectProvider` syncs it into a local emf.osgi `EObjectRegistry`, and `ModelAtlasBootstrap` resolves the reference against it (registry/key missing = transient: keep last good state, re-apply via `EObjectRegistryListener`; wrong type = fail hard). `configuration.ecore` references qvtoperational and query by nsURI now (never relative — the published-schema rule). Milestone 8 (DCAT publication, issue #4) is implemented: a `DcatPublication` (plain data, `publications` registry + `DataProvider.publication` reference, override-else-default) opts a provider into open-data publication; the omittable `publication.dcat` bundle maps it DataService-first onto the dcat.atlas client (register service -> datasets -> distributions -> links on EVERY sync, withdrawal on disappearance, retry for transient failures, portal refusal = non-retried ERROR state, one `PublicationStatus` service per provider), the bootstrap diagnoses declared-but-unhandled publications via the `data.atlas.publication.handler` marker, the portal endpoint is the client's Config-Admin factory config (mounted), the public base URL is `DATA_ATLAS_PUBLIC_BASE_URL`; the annotation-derived description (GenModel documentation of the DataSet's output type, used when the DataSet declares none) survives the Model Atlas round-trip (verified 2026-09-08 by `DcatPublicationAtlasModeIntegrationTest`). Bundles: `configuration.model`, `api`, `bootstrap`, `input.file`, `input.jpa`, `input.bridge`, `transformation`, `rest`, `publication.dcat`, `runtime.config` / `runtime.config.atlas` (resource-only Configurators, one per config-source flavour), `runtime` (bndruns; `_docker` = file mode, `_docker_atlas` = Model Atlas mode), `tests` (OSGi integration tests, H2-backed for JPA; docker-gated against the real DCAT.Atlas image), plus the docker modules (`docker/dataatlas`, `docker/dataatlas-atlas`, compose setups in `docker/dockercompose`, `docker-compose-dcat.yml` for the publication example). The old JPA data plane and `dcat.model` were removed in August 2026; the DCAT model used now is the portal's `org.eclipse.fennec.dcat.atlas.dcatap.de.model`, referenced ONLY by `publication.dcat`.
